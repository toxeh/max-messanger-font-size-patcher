...
func main() {
	binaryPath := flag.String("binary", "", "Path to max.exe (e.g. C:\\Users\\you\\AppData\\Local\\max\\max.exe)")
	...
	if *binaryPath == "" {
		fmt.Fprintln(os.Stderr, "Usage: font-patcher.exe -binary <path> -style <name> -size <px>")
		fmt.Fprintln(os.Stderr, "\nExamples:")
		fmt.Fprintln(os.Stderr, `  font-patcher.exe -binary "C:\Users\you\AppData\Local\max\max.exe" -style MarkdownMessageBase -size 12`)
		fmt.Fprintln(os.Stderr, `  font-patcher.exe -binary max.exe -style list`)
		os.Exit(1)
	}
	...
	for pos < len(data)-10 {
		...
		for _, targetStyle := range stylesToPatch {
			...
			// Find the end of this style block (next top-level style starts after }},")
